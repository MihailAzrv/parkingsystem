...
public class TicketDAO {
	...
	public boolean saveTicket(Ticket ticket) {
		...
		try {
			...
			ps.setTimestamp(5, (ticket.getOutTime() == null) ? null
					: (new Timestamp(ticket.getOutTime().toEpochSecond(ZoneOffset.UTC))));
			return ps.execute();
		} catch (Exception ex) {
	...
	public boolean isReccuringUser(String vehicleRegNumber) {
		...
		try {
			con = dataBaseConfig.getConnection();
			PreparedStatement ps = con.prepareStatement(DBConstants.IS_RECCURING_USER);
			ps.setString(1, vehicleRegNumber);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
			...
			logger.error("Error checking reccurent user", ex);
		} finally {
			dataBaseConfig.closeConnection(con);
		}
		return false;
